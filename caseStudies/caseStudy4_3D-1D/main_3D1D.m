...
C(size(faceBoundaryMarker(:),1)+1:size(C,1)) = 7*ones((size(C,1)-size(faceBoundaryMarker,1)),1);
...
inputStruct.holePoints=[V_region2; [475e-6 288e-6 180e-6]]; %Interior points for holes
...
%% Converst .msh to .xdmf via meshio
...
%% Projection back from 1D bars to 3D Surface Mesh
[D,minInd]=minDist(Vsurf,Vske); %minInd is the index of the vertices in the skeleton
%if C was a pressure field on the skeleton, C2 is the Pressure on the 3D vessel wall
...
faceAlpha=1;
cFigure;
patch('faces',Fsurf,'vertices',Vsurf,'FaceColor','flat','CData',oxygenSurfFaces,'FaceAlpha',faceAlpha,'EdgeColor','none');
axisGeom;
camlight ;
drawnow;
axis off
...
script2 = fileread('../../../core/FEniCS_3D-1D.py');

% Join Info.txt with 3D-1D FEniCS file in the core
